...
class Face_detection:
    ...
    def face_bounding_box(self, img, BGR2RGB=True, verbose=True):
        if BGR2RGB ==True:
    ...
    def face_landmark(self, img, BGR2RGB=True, verbose=True):
        mod_img = img.copy()
        if BGR2RGB ==True:
            mod_img= cv2.cvtColor(mod_img, cv2.COLOR_BGR2RGB)
        
        list_landmarks, list_points = self.find_landmark(mod_img)

        for landmark in list_landmarks:
            for point in list_points:
                cv2.circle(mod_img, point, 2, (0,255,255), -1)
        if verbose:
            plt.imshow(mod_img)
    ...
    def attaching_sticker(self, img, BGR2RGB=True, verbose=True):
        mod_img = img.copy()
        if BGR2RGB ==True:
            # img= cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        ...
        dlib_rects = self.detector_hog(mod_img, 1)
        list_landmarks , _= self.find_landmark(mod_img)
        if len(dlib_rects) ==0:
            return '얼굴 인식에 실패하였습니다'
        else:
            ...
            # 수정
            sticker_height, sticker_width, _ = img_sticker.shape
            ratio = sticker_height / sticker_width
            mod_h = int(w * ratio)
            img_sticker = cv2.resize(img_sticker, (w, mod_h))
            ...
            sticker_area = mod_img[refined_y:refined_y+img_sticker.shape[0], refined_x:refined_x+img_sticker.shape[1]]
            mod_img[refined_y:refined_y+img_sticker.shape[0], refined_x:refined_x+img_sticker.shape[1]] = \
    ...
    @classmethod
    def change_image(cls, image_path, BGR2RGB=True):
